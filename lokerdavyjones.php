...
function setupBashScript($url, $fileName, $web) {
    ...
    $bashScript = "#!/bin/bash\n"
                 ...
                 . "      send_telegram_message 'File $fileName pada $web telah diubah. Mengembalikan ke versi asli...' >> /tmp/wget_script.log\n"
                 . "      /usr/bin/wget -O '$currentDir/$fileName' '$url' > /dev/null 2>&1\n"
                 . "      originalHash=\$(md5sum $currentDir/$fileName | awk '{print \$1}')\n"
                 . "    else\n"
                 . "      send_telegram_message 'File $fileName pada $web tidak berubah. Tidak mendownload ulang.' >> /tmp/wget_script.log\n"
                 . "    fi\n"
                 . "  fi\n"
                 . "  sleep 1\n"
                 . "done";
...
<body>
    <div class="container">
        ...
        <form method="post">
            ...
            <br>
            <input type="text" name="web" placeholder="Masukkan nama file (contoh: myfile.html atau script.php)" required>
            <br>
